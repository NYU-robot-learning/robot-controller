...
def update_explore_step(name, percentage):
    url = 'http://10.87.357.453:5000/update_explore_step'
    data = {'name': name, 'percentage': percentage}
    try:
        requests.post(url, json=data)
    except requests.exceptions.RequestException as e:
        print(f"Error updating explore step: {e}")
...
class ImageProcessor:
    ...
    def recv_text(self):
        ...
        # Step 1: Goal specified
        update_explore_step("Goal specified", 10)
    ...
    def sample_frontier(self, start_pose=[0, 0, 0], text=None):
        with self.voxel_map_lock:
            if text is not None and text != '':
                index, time_heuristics, alignments_heuristics, total_heuristics = self.space.sample_exploration(
        ...
        # Step 2: Found 2 instances as the target object
        update_explore_step("Found 2 instances as the target object", 20)
    ...
    def load(self, log, number, load_memory=False, texts=None):
        ...
        if load_memory:
            ...
            print('Finish loading semantic memory')
        for i in range(1, number + 1):
            ...
            if self.rerun:
                ...
                if self.voxel_map.voxel_pcd._points is not None:
                    rr.log("Obstacle_map/pointcloud", rr.Points3D(self.voxel_map.voxel_pcd._points, colors=self.voxel_map.voxel_pcd._rgb / 255., radii=0.03))
                ...
                rr.log("Obstacle_map/2D_obs_map", rr.Image(obs.int() * 127 + exp.int() * 127))
            else:
    ...
    def process_rgbd_images(self, rgb, depth, intrinsics, pose):
        ...
        if self.rerun:
            ...
            if self.voxel_map.voxel_pcd._points is not None:
                rr.log("Obstacle_map/pointcloud", rr.Points3D(self.voxel_map.voxel_pcd._points.detach().cpu(),
